...
$liveid_appid	= get_option('login_buttons_liveid_appid_key');
$liveid_secret	= get_option('login_buttons_liveid_secret_key');
$liveid_secalgo	= 'wsignin1.0';
$liveid_return	= login_buttons_PLUGIN_URL . '/liveid/callback.php';
$liveid_policy	= get_option('login_buttons_liveid_policy_url');
...
if ($consenttoken) {
  ...
  if($xml) {
    ...
    $signature = login_buttons_generate_signature($liveid);
...
<script>
function init() {
  window.opener.wp_login_buttons({'action' : 'login_buttons', 'login_buttons_provider' : 'liveid', 
    'login_buttons_signature' : '<?php echo $signature ?>',
    'login_buttons_liveid_identity' : '<?php echo $liveid ?>',
