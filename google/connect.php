...
try {
  if(!isset($_GET['openid_mode']) || $_GET['openid_mode'] == 'cancel') {
    ...
    if($openid->validate()) {
      ...
      $signature = login_buttons_generate_signature($google_id);
      ?>
<html>
<head>
<script>
function init() {
  window.opener.wp_login_buttons({'action' : 'login_buttons', 'login_buttons_provider' : 'google', 
    'login_buttons_openid_identity' : '<?php echo $google_id ?>',
